	...
	def init_encoder_seq(self, example_list, hps):
		...
		if hps.word_gcn.value:
			...
			if hps.use_coref_graph.value:
				self.word_adj_in_coref, self.word_adj_out_coref = data.get_specific_adj(edge_list, hps.batch_size.value, max_enc_seq_len, 'coref', keep_prob=hps.word_gcn_edge_dropout.value)
			
			if hps.use_entity_graph.value:
				_, self.word_adj_entity = data.get_specific_adj(edge_list, hps.batch_size.value, max_enc_seq_len, 'entity', use_both=False, keep_prob=hps.word_gcn_edge_dropout.value)

			if hps.use_lexical_graph.value:
				_, self.word_adj_lexical = data.get_specific_adj(edge_list, hps.batch_size.value, max_enc_seq_len, 'lexical', use_both=False, keep_prob=hps.word_gcn_edge_dropout.value)
