...
class SummarizationModel(object):
    ...
    def _add_gcn_layer(self, gcn_in, in_dim, gcn_dim, batch_size, max_nodes, max_labels, adj_in, adj_out,
        ...
        for layer in range(num_layers):
            ...
            with tf.variable_scope('%s-%d' % (name, layer)):
                ...
                h_in = h_in + labels
                h_in = tf.reshape(h_in, tf.shape([batch_size, self._max_word_seq_len, -1]))
                ...
                h_out = tf.reshape(h_out, tf.shape([batch_size, self._max_word_seq_len, -1]))

                # graph convolution, loops
                h_loop = tf.matmul(gcn_in_2d, w_loop) + b_loop
                h_loop = h_loop * gates_loop
                h_loop = tf.reshape(h_loop, tf.shape([batch_size, self._max_word_seq_len, -1]))
    ...
    def _add_gcn_layer_2(self, gcn_in, in_dim, gcn_dim, batch_size, max_nodes, max_labels, adj_in, adj_out,
                       neighbour_count, num_layers=1,
                       use_gating=False, use_skip=True, use_normalization=True, dropout=1.0, name="GCN",
                       use_label_information=False):  # output becomes gcn_in ka dimension
        ...
        for layer in range(num_layers):
            ...
            with tf.variable_scope('%s-%d' % (name, layer)):
                ...
                w_loop = tf.get_variable('w_loop', [in_dim, gcn_dim],
                                         initializer=tf.contrib.layers.xavier_initializer(),
                                         regularizer=self._regularizer)
                b_layer = tf.get_variable('b_layer', [1], initializer=tf.constant_initializer(0.0),
                                          regularizer=self._regularizer)  # gating for the highway networks
                ...
                for lbl in range(max_labels):
                    ...
                    with tf.variable_scope('label-%d_name-%s_layer-%d' % (lbl, name, layer)) as scope:
                        ...
                        # Edge gating
                        if use_gating:
                            b_labels = tf.get_variable('label-%d_name-%s' % (lbl, name), [1],
                        ...
                        if use_gating:
                            ...
                            b_gin = tf.get_variable('b_gin', [1], initializer=tf.constant_initializer(0.0),
                                                    regularizer=self._regularizer)
                            b_gloop = tf.get_variable('b_gloop', [1], initializer=tf.constant_initializer(0.0),
                                                      regularizer=self._regularizer)

                        if use_skip:
                            w_skip = tf.get_variable('w_skip', [gcn_dim, true_in_dim],
                                                     initializer=tf.contrib.layers.xavier_initializer(),
                                                     regularizer=self._regularizer)
    ...
    def _add_seq2seq(self):
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('embedding'):
                ...
                if hps.query_encoder:
                    emb_query_inputs = tf.nn.embedding_lookup(embedding,
                                                              self._query_batch)  # tensor with shape (batch_size, max_query_steps, emb_size)
            ...
            if self._hps.concat_with_word_embedding:  # intermediate concat
                w_word = tf.get_variable('w_word',
                                         [self._hps.hidden_dim * 2 + self._hps.emb_dim, self._hps.hidden_dim * 2],
                                         dtype=tf.float32, initializer=self.trunc_norm_init, trainable=True,
                                         regularizer=self._regularizer)
                b_word = tf.get_variable('b_word', [1, self._hps.hidden_dim * 2],
                                         initializer=tf.constant_initializer(0.0), regularizer=self._regularizer)
            ...
            if self._hps.word_gcn:
                ...
                if self._hps.use_gcn_lstm_parallel:
                    ...
                    if self._hps.concat_with_word_embedding:  # interm concat
                        ...
                        if hps.emb_dim != hps.hidden_dim * 2:
                            w_adjust = tf.get_variable('w_adjust', [hps.emb_dim, hps.hidden_dim * 2],
                                                       initializer=tf.contrib.layers.xavier_initializer(),
                                                       regularizer=self._regularizer)
                ...
                if self._hps.concat_gcn_lstm and self._hps.word_gcn:  # upper concat
                    ...
                    if hps.word_gcn_dim != hps.hidden_dim * 2:
                        w_adjust_upper_concat = tf.get_variable('w_adjust_upper_concat',
                                                                [hps.word_gcn_dim, hps.hidden_dim * 2],
                                                                initializer=tf.contrib.layers.xavier_initializer(),
                                                                regularizer=self._regularizer)
            ...
            if self._hps.query_encoder:
                if self._hps.no_lstm_query_encoder:
                    ...
                    query_outputs, fw_st_q, bw_st_q = self._add_encoder(emb_query_inputs, self._query_lens,
                                                                        name='query_encoder')
                ...
                if self._hps.query_gcn:
                    ...
                    if self._hps.concat_gcn_lstm and self._hps.query_gcn:  # deprecated
                        if self._hps.simple_concat:
                            ...
                            self._query_states = tf.nn.relu(tf.add(tf.multiply(q_gcn_outputs, q_gcn_word_w),
                                                                   tf.multiply(query_outputs, q_lstm_word_w)))
    ...
    def _add_gcn_seq2seq(self):
        ...
        with tf.variable_scope('gcn_seq2seq'):
            ...
            if hps.concat_with_word_embedding:  #
                ...
                w_word = tf.get_variable('w_word', [hps.word_gcn_dim + self._hps.emb_dim, hps.word_gcn_dim],
                                         dtype=tf.float32, initializer=self.trunc_norm_init, trainable=True,
                                         regularizer=self._regularizer)
                b_word = tf.get_variable('b_word', [1, hps.word_gcn_dim], initializer=tf.constant_initializer(0.0),
                                         regularizer=self._regularizer)
            ...
            if self._hps.concat_gcn_lstm:
                w_gcn_lstm = tf.get_variable('w_gcn_lstm', [hps.word_gcn_dim + hps.hidden_dim * 2, hps.hidden_dim * 2],
                                             dtype=tf.float32, initializer=self.trunc_norm_init, trainable=True,
                                             regularizer=self._regularizer)
                b_gcn_lstm = tf.get_variable('b_gcn_lstm', [1, hps.hidden_dim * 2],
                                             initializer=tf.constant_initializer(0.0), regularizer=self._regularizer)
            ...
            if self._hps.concat_gcn_lstm and self._hps.word_gcn:
                if self._hps.simple_concat:
                    ...
                    self._enc_states = tf.nn.relu(
                        tf.add(tf.tensordot(interm_outputs_2, w_gcn_lstm, axes=[[2], [0]]), b_gcn_lstm))
            ...
            if self._hps.query_encoder:
                ...
                query_outputs, fw_st_q, bw_st_q = self._add_encoder(q_gcn_outputs, self._query_lens,
                                                                    name='query_encoder')
                ...
                if self._hps.concat_gcn_lstm and self._hps.query_gcn:
                    if self._hps.simple_concat:
                        ...
                        self._query_states = tf.add(tf.multiply(q_gcn_outputs, q_gcn_word_w),
                                                    tf.multiply(query_outputs, q_lstm_word_w))
